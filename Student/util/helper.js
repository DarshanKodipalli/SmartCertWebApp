...
module.exports.sendAuthReq = function sendAuthenticationRequest(req){
    ...
    var reqMade = http.request(options,function(res){
        console.log(res.statusCode);
        var buffer;
        reqMade.on( "data", function( data ) {
            buffer = buffer + data;
            console.log('data');
        });
        reqMade.on( "end", function( data ) {
            console.log( buffer );
            console.log('print buffer');
        });
    });

    reqMade.on('error',function(err){
       console.log(err.message);
    });

    console.log('about to send');
    console.log(finalRequestXml);
    reqMade.write(finalRequestXml);
    reqMade.end();
