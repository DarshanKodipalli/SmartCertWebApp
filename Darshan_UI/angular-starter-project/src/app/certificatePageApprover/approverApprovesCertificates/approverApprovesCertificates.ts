...
export class CertificatesToBeApproved implements OnInit,OnDestroy{
	...
	constructor(private http:RestCallsComponent){
		...
		console.log(JSON.parse(localStorage.getItem("batchInf")));
		this.receivedCertificateParams = JSON.parse(localStorage.getItem("batchInf"));
		this.selectedBatch = JSON.parse(localStorage.getItem("batchInf")).year;
		this.selectedCourse = JSON.parse(localStorage.getItem("batchInf")).courseName;
		this.initSub = this.http.getCertificatesForBatchAndInstitutionAndUniversityForApprover(this.receivedCertificateParams).subscribe(
			(response)=>{
